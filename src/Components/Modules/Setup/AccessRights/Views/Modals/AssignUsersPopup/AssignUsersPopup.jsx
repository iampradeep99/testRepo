...
import AssignUserPopUpLogic from "./Logic/Logic";
import UnAssignUsersPopup from "../UnAssignUsersPopUp/UnAssignUsersPopUp";
import BizClass from "../../../../ProfileManagement/Views/Modals/AssignedUserListModal/AssignedUserListModal.module.scss";
...
function AssignUsersPopup({ showfunc, assignedUsersPopUp }) {
  const {
    unAssignUsersPopUp,
    ...
    deletedAssignedUser,
  } = AssignUserPopUpLogic();
  useEffect(() => {
    getAssignedUserListData(assignedUsersPopUp);
  }, [assignedUsersPopUp]);
  ...
  return (
    <>
      {unAssignUsersPopUp && (
        <UnAssignUsersPopup
          showfunc={toggleUnAssignUsersPopUp}
          assignedUsersPopUp={assignedUsersPopUp}
      ...
      <Modal
        varient="half"
        title={`Assigned User List (${assignedUsersPopUp.RightName ? assignedUsersPopUp.RightName : ""})`}
        show={showfunc}
        right="0"
        width="49.5vw"
      >
        <Modal.Body>
          <div className={BizClass.Card}>
            <PageBar>
              <PageBar.Search value={searchTextAssigendUser} onChange={(e) => onSearchAssignedUser(e.target.value)} />
              <PageBar.Button onClick={() => toggleUnAssignUsersPopUp()}>Import</PageBar.Button>
            </PageBar>
            <DataGrid
...
AssignUsersPopup.propTypes = {
  ...
  assignedUsersPopUp: PropTypes.object,
};
